...
void EvaluationDataCollectorImpl<SlotIdType>::pushData(
    ...
    const DataType& data) {
  // LOG(INFO) << "Pushing data of slot id " << std::to_string(slot_id) << " to
...
template <class SlotIdType>
typename EvaluationDataCollectorImpl<SlotIdType>::ChannelGroup*
EvaluationDataCollectorImpl<SlotIdType>::getSlot(const SlotId& slot_id) {
...
template <class SlotIdType>
const typename EvaluationDataCollectorImpl<SlotIdType>::ChannelGroup*
EvaluationDataCollectorImpl<SlotIdType>::getSlot(const SlotId& slot_id) const {
  // CHECK(slot_id.isValid());
  std::lock_guard<std::mutex> lock(m_channel_groups_);
...
template <class SlotIdType>
typename EvaluationDataCollectorImpl<SlotIdType>::ChannelGroup*
EvaluationDataCollectorImpl<SlotIdType>::getSlotAndCreateIfNecessary(
...
template <class SlotIdType>
void EvaluationDataCollectorImpl<SlotIdType>::removeSlotIfAvailable(
    const SlotId& slot_id) {
  // CHECK(slot_id.isValid());
  std::lock_guard<std::mutex> lock(m_channel_groups_);
...
template <class SlotIdType>
bool EvaluationDataCollectorImpl<SlotIdType>::hasSlot(
    const SlotId& slot_id) const {
  return (getSlot(slot_id) != nullptr);
}

template <class SlotIdType>
bool EvaluationDataCollectorImpl<SlotIdType>::hasChannel(
    const SlotId& slot_id, const std::string& channel_name) const {
  const ChannelGroup* slot;
  if ((slot = getSlot(slot_id)) == nullptr) {
    return false;
