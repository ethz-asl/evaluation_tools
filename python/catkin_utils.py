...
def getCatkinConfig(profile="default"):
    ...
    config_yaml_path = os.path.join(ws_path, CATKIN_TOOLS_SUBF, profile, "config.yaml")
    if not os.path.exists(config_yaml_path):
        config_yaml_path = os.path.join(ws_path, CATKIN_TOOLS_SUBF, "profiles", profile, "config.yaml")
    if not os.path.exists(config_yaml_path):
        raise ValueError("config_yaml_path does not exist: '{}'".format(config_yaml_path))
    file = open(config_yaml_path)
    config = yaml.safe_load(file)
    file.close()
    return config
    
def catkinFind(package_name):
    return subprocess.check_output(["catkin_find", package_name]).decode('ascii').split()
...
def catkinFindSubfolder(package_name, req_sub_folder):
    ...
    for folder in folder_list:
        if req_sub_folder in folder:
            return folder.strip();
    raise ValueError("Could not find " + req_sub_folder + " folder for the package " + package_name)
...
def catkinFindCamCalib(ze_cam_id):
    ...
    for folder in ze_calib_folders:
        ...
        if os.path.exists(cam_calib_folder) and os.path.isdir(cam_calib_folder):
            return cam_calib_folder
    raise ValueError("Could not find calibration folder for ZE camera id " + str(ze_cam_id))
...
def catkinFindExperimentsFolder():
    ...
    for folder in ze_test_data_folders:
        ...
        if os.path.exists(experiments_folder) and os.path.isdir(experiments_folder):
            return folder
    return ''

def getRevString(cwd_folder):
    rev_cmd = ["git", "rev-parse","HEAD"]
    out_lines = subprocess.check_output(rev_cmd, cwd=cwd_folder).decode('ascii').split()
    if len(out_lines) != 1:
