...
class Plotter(object):
  ...
  def plot(self, metrics):
    ...
    Creates one Data namedtuple that contains the data from all runs, except
    those that come from parameter sweeps, and one SweepData namedtuple for each
    parameter sweep. Then calls the plotting functions for each Data and
    SweepData.
    """
    for metric, parameter_files in metrics.items():
      ...
      for parameter_sweep_file in parameter_sweep_files:
        sweep_data_dict[parameter_sweep_file] = self.SweepData(
            label=metric, indices=[], means=[], stddevs=[], mins=[], maxes=[])
  ...
  def plotDataWithoutSweeps(self, data):
    ...
    for i in range(N):
      ...
      patches.append(
          mpatches.Patch(color=current_color, label=data.parameter_files[i]))
  ...
  def plotSweepsData(self, data):
    ...
    for param_file, sweep_data in data.items():
      ...
      plt.figure()
      plt.title(
          '{} - param sweep from {}'.format(sweep_data.label, param_file),
          fontsize=14)
    ...
    for file in files_to_summarize:
      ...
      for metric, values in metrics.items():
        self.metrics[(dataset,
                      parameter_file)][metric].addSampleFromDict(values)

    self.createMetricsIndices()
    self.logger.info(
        "Extracted data from {} runs, across {} datasets and {} parameter sets."
        "".format(
            len(self.files_to_summarize), len(self.datasets),
  ...
  def createMetricsIndices(self):
    ...
    for dataset in self.datasets:
      for parameter_file in self.parameter_files:
        self.metrics_by_dataset[dataset][parameter_file] = self.metrics[(
            dataset, parameter_file)]

    self.metrics_by_parameter_file = defaultdict(dict)
    for parameter_file in self.parameter_files:
      for dataset in self.datasets:
        self.metrics_by_parameter_file[parameter_file][dataset] = self.metrics[(
            dataset, parameter_file)]
